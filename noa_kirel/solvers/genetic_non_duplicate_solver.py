...
class GeneticNonDuplicatesSolver(Solver):
    """
    A class for a Genetic algorithm solver for the tour planning problem with no duplicates in tour
    """
    ...
    @staticmethod
    def __sols_with_duplicates(population):
        return np.array([i for i, sol in enumerate(population) if len(np.unique(sol)) < len(sol)])
    ...
    def __mutation(self, population):
        ...
        """
        random_idxs_to_be_mutated = np.argwhere(np.random.choice([0, 1], size=len(population),
                                                                 p=[1 - self.__mutate_p, self.__mutate_p])).flatten()
        random_genes_to_be_mutated = population[random_idxs_to_be_mutated]
        for i in range(len(random_genes_to_be_mutated)):
            random_mutation_idx = np.random.randint(self.__tour_length)
            mutation_city = self.__city_selection_func(population, self.__num_cities)
            gen = random_genes_to_be_mutated[i]
            gen[random_mutation_idx] = mutation_city
        ...
        for i in range(len(genes_to_be_mutated)):
            ...
            for idx in duplicates:
                forbidden = {gen[idx + 1]} if idx == 0 \
                    else {gen[idx - 1], gen[idx + 1]} if idx < self.__tour_length - 1 \
                    else {gen[idx - 1]}
                cities = {city for city in range(self.__num_cities)} - forbidden
                gen[idx] = random.sample(cities, 1)[0]
            population[duplicate_idxs[i]] = gen
        return population
